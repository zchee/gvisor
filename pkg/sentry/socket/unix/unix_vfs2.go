...
import (
	...
	"gvisor.dev/gvisor/pkg/sentry/fsimpl/socketfs"
...
func NewVFS2File(ctx context.Context, vfsObj *vfs.VirtualFilesystem, ep transport.Endpoint, stype linux.SockType) (*vfs.FileDescription, *syserr.Error) {
	sock := NewFDImpl(ep, stype)
	// File mode matches net/socket.c:sock_alloc.
	d := socketfs.NewDentry(ctx, vfsObj, 0600)
	vfsfd := &sock.vfsfd

	opts := &vfs.FileDescriptionOptions{UseDentryMetadata: true}
	if err := vfsfd.Init(sock, linux.O_RDWR, vfsObj.SocketMount, d, opts); err != nil {
		return nil, syserr.FromError(err)
	}
	return vfsfd, nil
}

// NewFDImpl creates and returns a new SocketVFS2 for a unix socket.
...
// VFSFD retrives the vfs.FileDescription containing to s.
...
func (s *SocketVFS2) Accept(t *kernel.Task, peerRequested bool, flags int, blocking bool) (int32, linux.SockAddr, uint32, *syserr.Error) {
	...
	ns, err := NewVFS2File(t, t.Kernel().VFS(), ep, s.stype)
	if err != nil {
		return 0, nil, 0, err
	}
	defer ns.DecRef()
...
func (s *SocketVFS2) Bind(t *kernel.Task, sockaddr []byte) *syserr.Error {
	...
	return s.ep.Bind(tcpip.FullAddress{Addr: tcpip.Address(p)}, func() *syserr.Error {
		...
		if p[0] == 0 {
			...
			e := t.Kernel().VFS().BindAt(t, t.Credentials(), &pop, bep, linux.FileMode(0400))
...
func (s *SocketVFS2) Release() {
	// TODO: in vfs1, we just do a DecRef() on the file because the abstract
	// socket namespace might be holding a ref? What does that mean?
	s.DecRef()
	//s.ep.Close()
}
...
func (*providerVFS2) Socket(t *kernel.Task, stype linux.SockType, protocol int) (*vfs.FileDescription, *syserr.Error) {
	...
	return NewVFS2File(t, t.Kernel().VFS(), ep, stype)
}
...
func (*providerVFS2) Pair(t *kernel.Task, stype linux.SockType, protocol int) (*vfs.FileDescription, *vfs.FileDescription, *syserr.Error) {
	...
	s1, err := NewVFS2File(t, t.Kernel().VFS(), ep1, stype)
	if err != nil {
		return nil, nil, err
	}
	s2, err := NewVFS2File(t, t.Kernel().VFS(), ep2, stype)
	if err != nil {
		return nil, nil, err
	}
