...
func forEachMount(t *kernel.Task, fn func(string, *vfs.Mount)) {
	...
	// will be excluded.
	vfsRoot := fsctx.RootDirectoryVFS2()
	defer vfsRoot.DecRef()
	rootMnt := vfsRoot.Mount()
	if rootMnt == nil {
	...
	// Collect this mount and all children.
	allMounts := append([]*vfs.Mount{rootMnt}, rootMnt.ChildMounts()...)

	// Run the function on each mount.
	for i, mnt := range allMounts {
		mroot := mnt.Root()
		path, err := t.Kernel().VFS().PathnameWithDeleted(t, vfsRoot, mroot)
		if err == nil {
			fn(path, mnt)
		}
		mroot.DecRef()
...
func (i *mountInfoData) Generate(ctx context.Context, buf *bytes.Buffer) error {
	vfsObj := i.task.Kernel().VFS()
	forEachMount(i.task, func(mountPath string, mnt *vfs.Mount) {
		mroot := mnt.Root()
		defer mroot.DecRef()

		// Stat the root, we need some info from it.
		pop := &vfs.PathOperation{
			Root:  mroot,
			Start: mroot,
			Path:  fspath.Parse(""),
		}
		statx, err := vfsObj.StatAt(ctx, auth.NewAnonymousCredentials(), pop, &vfs.StatOptions{
			Mask: linux.STATX_ALL,
		})
		if err != nil {
		...
		// (9) Filesystem type.
		fmt.Fprintf(buf, "%s ", mnt.Filesystem().FilesystemType().Name())

		// (10) Mount source: filesystem-specific information or "none".
		fmt.Fprintf(buf, "none ")

		// (11) Superblock options, and final newline.
		fmt.Fprintf(buf, "%s\n", superBlockOpts(mountPath, mnt))
	})
	return nil
}

func superBlockOpts(mountPath string, mnt *vfs.Mount) string {
	...
	// path.
	if mnt.Filesystem().FilesystemType().Name() == "cgroup" {
...
func (i *mountsData) Generate(ctx context.Context, buf *bytes.Buffer) error {
	forEachMount(i.task, func(mountPath string, mnt *vfs.Mount) {
		...
		fmt.Fprintf(buf, "%s %s %s %s %d %d\n", "none", mountPath, "foo", opts, 0, 0)
		fmt.Fprintf(buf, "%s %s %s %s %d %d\n", "none", mountPath, mnt.Filesystem().FilesystemType().Name(), opts, 0, 0)
	})
	return nil
}
