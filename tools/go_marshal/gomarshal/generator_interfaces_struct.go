...
func (g *interfaceGenerator) emitMarshallableSliceForStruct(st *ast.StructType, slice *sliceAPI) {
	...
	g.inIndent(func() {
		...
		fallback := func() {
			...
			g.emit("for idx, limit := 0, length/size; idx < limit; idx++ {\n")
			g.inIndent(func() {
	...
	g.inIndent(func() {
		...
		if thisPacked {
			...
			g.emit("length, err := safecopy.CopyIn(dst[:(size*count)], unsafe.Pointer(val))\n")
			g.emit("// Since we bypassed the compiler's escape analysis, indicate that src\n")
	...
	g.inIndent(func() {
		...
		if thisPacked {
			...
			g.emit("length, err := safecopy.CopyOut(unsafe.Pointer(val), src[:(size*count)])\n")
			g.emit("// Since we bypassed the compiler's escape analysis, indicate that src\n")
