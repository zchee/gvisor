...
type mockTask struct {
	taskMem []byte
	// How many bytes of I/O to allow. If limit is lower than the requested I/O
	// size, the I/O operation is truncated with an error. Limit of 0 implies no
	// limits.
	limit int
}
...
func (t *mockTask) CopyInBytes(_ usermem.Addr, b []byte) (int, error) {
	...
	limitedTaskMem := t.taskMem
	if len(limitedTaskMem) > limit {
		limitedTaskMem = limitedTaskMem[:limit]
	}

	n := copy(b, limitedTaskMem)
